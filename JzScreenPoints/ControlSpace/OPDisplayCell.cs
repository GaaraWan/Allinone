...
{
    class OPDisplayCell : OPDisplayNormal
    {
        ...
        OPTypeEnum OPType = OPTypeEnum.ASN;
        Pen PKeyBorder = new Pen(Color.Yellow, 3);

        int FirstCellIndex = -1;
        int CellSelectCount = 0;
        bool IsMultiSelect = false;
        string SelectionStr = "";
        bool IsUseHotkeyToResize = false;
        bool IsHotkeytoResize = false;
        ...
        //public void SetENHANCEList(List<ENHANCEClass> enhancelist)
        //{
                ...
                switch (DISPOPType)
                {
                    case DisplayOPTypeEnum.SELECT:
                        ...
                        else
                        {
                            if (IsMultiSelect)
                            {
                                if (SelectionStr.IndexOf(cell.RelateIndexNoStr) < 0)
                                    cell.SetNoSelected();
                            }
                            else
                            {
        ...
        */
        ...
        public void UseHotKeyToResize(bool isuse)
        {
            IsUseHotkeyToResize = isuse;
        }
        public void HotKeyToResize(bool ishotkey)
        {
            IsHotkeytoResize = ishotkey;
        }
        ...
        public void MultiSelectionSet()
        {
            ...
            foreach (CellClass cell in CELLList)
            {
                if (cell.IsSelected)
                {
                    SelectionStr += cell.RelateIndexNoStr + ",";
                }
            }
        ...
        void DrawInViewPortDomain(CellClass cell, bool IsBasDatum)
        {
            ...
            switch (Universal.VER)
            {
                case VersionEnum.None:
                    Rectangle circlerect = cell.CircleCell.ToRect();

                    circlerect.X = circlerect.X - JzTools.ShiftValue(ptViewPortLocationLive.X, iViewPortRatio);
                    circlerect.Y = circlerect.Y - JzTools.ShiftValue(ptViewPortLocationLive.Y, iViewPortRatio);
                ...
                case CellPropertyEnum.STATIC:
                    ...
                    switch (Universal.VER)
                    {
                        case VersionEnum.None:
                            ...
                            DISPUI.DrawCircle(JzTools.GetRectCenter(rect), rect.Width >> 1, new SolidBrush(cell.PBorder.Color));
                            DISPUI.DrawCircle(JzTools.GetRectCenter(rect), rect.Width >> 1, new SolidBrush(cell.POperate.Color));
