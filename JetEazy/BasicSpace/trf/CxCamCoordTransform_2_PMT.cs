...
namespace JetEazy.Aoi.TransformD
{
    ...
    {
        //const MatrixType MAT_TYPE = MatrixType.F32C1;
        MatrixType MAT_TYPE = MatrixType.CV_32FC1;
        ...
        public void TransformWorldToView(QVector pointWorld, out QVector pointView)
        {
            ...
            //////using (CvMat w = new CvMat(3, 1, MAT_TYPE))
            //////using (CvMat v = new CvMat(3, 1, MAT_TYPE))
            //////{
            //////}
            //////w[0, 0] = pointWorld.x;
            //////w[1, 0] = pointWorld.y;
            ...
            double x;
            double y;
            //using (CvMat src = new CvMat(1, 1, MatrixType.F32C2))
            //using (CvMat dst = new CvMat(1, 1, MatrixType.F32C2))
            using (CvMat src = new CvMat(1, 1, MatrixType.CV_32FC2))
            using (CvMat dst = new CvMat(1, 1, MatrixType.CV_32FC2))
            {
        ...
        public void TransformViewToWorld(QVector pointView, out QVector pointWorld)
        {
...
#else
            //////CvMat v = new CvMat(3, 1, MAT_TYPE);
            //////v[0, 0] = pointView.x;
            //////v[1, 0] = pointView.y;
            //////v[2, 0] = 1.0;
            //////CvMat w = mat * v;
            ...
            using (CvMat dst = new CvMat(1, 1, MatrixType.F32C2))
            {
                ...
                Cv.PerspectiveTransform(src, dst, mat);
        ...
        #region PRIVATE_FUNCTIONS
            ...
            {
                System.Diagnostics.Trace.Assert(MAT_TYPE == MatrixType.F32C1);
                ...
                for (int i = 0; i < m_iRows - 1; i++)
                {
                    for (int j = 0; j < m_iCols - 1; j++)
                    {
                        ...
                        if (m_matriceW2V[i, j] == null)
                            m_matriceW2V[i, j] = new CvMat(2, 4, MAT_TYPE, m_arrUnitMatData32F, true);

                        if (m_matriceV2W[i, j] == null)
                            m_matriceV2W[i, j] = new CvMat(2, 4, MAT_TYPE, m_arrUnitMatData32F, true);
#else
                        if (m_matriceW2V[i, j] == null)
                            m_matriceW2V[i, j] = CvMat.Identity(3, 3, MAT_TYPE);

                        if (m_matriceV2W[i, j] == null)
                            m_matriceV2W[i, j] = CvMat.Identity(3, 3, MAT_TYPE);
#endif
            ...
            private void _unifyMatrix(CvMat mat)
            {
            ...
            private void _assertUnifyMatrix(CvMat mat)
            {
            ...
            private void _buildMatrix(out CvMat matT, QVector[] ptsV, QVector[] ptsW)
            {
#if(OPT_USING_BI_LINEAR)
                ...
                CvMat mat1XY = new CvMat(4, 4, MAT_TYPE);
                CvMat matVX = new CvMat(4, 1, MAT_TYPE);
                CvMat matVY = new CvMat(4, 1, MAT_TYPE);
                ...
                CvMat matInv = new CvMat(4, 4, MAT_TYPE);
                ...
                CvPoint2D32f[] src = new CvPoint2D32f[len];
                CvPoint2D32f[] dst = new CvPoint2D32f[len];
                for (int i = 0; i < len; i++)
                {
                    src[i] = new CvPoint2D32f(ptsW[i].x, ptsW[i].y);
                    dst[i] = new CvPoint2D32f(ptsV[i].x, ptsV[i].y);
                }
