...
namespace JetEazy.BasicSpace
{
    ...
    public class HistogramClass
    {
        ...
        public int GetMinRatioAVG(float ratio)
        {
            ...
            int RatioIndex = (int)((float)TotalPixels * ratio);
            int RatioCount = 0;
            //int iTempCount = (int)(OriginSortingBars.Length * ratio);
    ...
    public class JzFindObjectClass
    {
        ...
        public Rectangle GetRectExpectAround(Rectangle myRect) //Get The Whole Rectangle
        {
            int i = 0;
            Rectangle Rect = new Rectangle();
            ...
            while (i < FoundList.Count)
            {
                if (FoundList[i].rect.Left == myRect.Left)
                {
                }
                else if (FoundList[i].rect.Right == myRect.Right)
                {
                }
                else if (FoundList[i].rect.Top == myRect.Top)
                {
                }
                else if (FoundList[i].rect.Bottom == myRect.Bottom)
                {
                }
                else
                {
        ...
        //public Rectangle GetRectExpectAround(RectangleF myRectF, double LRThreshold) //Get The Whole Rectangle
        //{
        //    return GetRectExpectAround(myRectF, 10, LRThreshold);
        //}
        public Rectangle GetRectExpectAround(RectangleF myRectF, double AreaThreshold, double LRThreshold, int isExpectTop = 0) //Get The Whole Rectangle
        {
            int i = 0;
            Rectangle Rect = new Rectangle();
            ...
            while (i < FoundList.Count)
            {
                ...
                //Point ptCenter = GetRectCenter(FoundList[i].rect);
                //Rectangle recttmp = SimpleRect(ptCenter);
                //myRectF.Inflate(-10, -10);
                //if(myRectF.IntersectsWith(recttmp))
                //{
                //    if (FoundList[i].Area > AreaThreshold)
                //        Rect = MergeTwoRects(Rect, FoundList[i].rect);
                //}
                ...
                RectangleF rectFTmp = new RectangleF(FoundList[i].rect.X, FoundList[i].rect.Y, FoundList[i].rect.Width, FoundList[i].rect.Height);
                rectFTmp.X += myRectF.X;
                rectFTmp.Y += myRectF.Y;


                if (Math.Abs(rectFTmp.Left - myRectF.Left) < LRThreshold)
                {
                }
                else if (Math.Abs(rectFTmp.Right - myRectF.Right) < LRThreshold)
                {
                }
                else if (Math.Abs(rectFTmp.Top - myRectF.Top) < LRThreshold && (isExpectTop == 1))
                {
                }
                else if (Math.Abs(rectFTmp.Bottom - myRectF.Bottom) < LRThreshold && (isExpectTop == 2))
                {
                }
                else
                {
        ...
        public Rectangle GetRectExpectAround_H(RectangleF myRectF, double AreaThreshold, double LRThreshold, int isExpectTop = 0) //Get The Whole Rectangle
        {
            int i = 0;
            Rectangle Rect = new Rectangle();
            ...
            while (i < FoundList.Count)
            {
                ...
                if (Math.Abs(rectFTmp.Left - myRectF.Left) < LRThreshold && (isExpectTop == 1))
                {
                }
                else if (Math.Abs(rectFTmp.Right - myRectF.Right) < LRThreshold && (isExpectTop == 2))
                {
                }
                else if (Math.Abs(rectFTmp.Top - myRectF.Top) < LRThreshold )
                {
                }
                else if (Math.Abs(rectFTmp.Bottom - myRectF.Bottom) < LRThreshold )
                {
                }
                else
                {
        ...
        public void Find(Bitmap bmp, Point PtStart, Color FillColor)
        {
            ...
            try
            {
                unsafe
                {
                    ...
                    if (pucStart[0] == 0xFF) //<== 窽ゎ恶屡︹!!!!度厚独ノ耕
                    {
                        Reset(x, y);
        ...
        public void FindGrayscale(Bitmap bmp, Color FillColor, int BaseColor, int ColorRange,int BaseRange)
        {
            ...
            try
            {
                unsafe
                {
                    ...
                    while (y < ymax)//if (pucStart[0] == 0xFF) //<== 窽ゎ恶屡︹!!!!度厚独ノ耕
                    {
                        x = xmin;
                        pucPtr = pucStart;
        ...
        public PointF GetBoraderPoint(Bitmap bmp, bool IsXdir, bool IsReverse, Color stopcolor)
        {
            Rectangle rectbmp = SimpleRect(bmp.Size);
            BitmapData bmpData = bmp.LockBits(rectbmp, ImageLockMode.ReadWrite, PixelFormat.Format32bppArgb);
            IntPtr Scan0 = bmpData.Scan0;
                    ...
                    if (IsXdir)
                    {
                        if (!IsReverse)
                        {
                            ...
                            while (y < ymax)
                            {
                                ...
                                while (x < xmax)
                                {
                                    if (pucPtr[2] == stopcolor.R && pucPtr[1] == stopcolor.G && pucPtr[0] == stopcolor.B)
                                    {
                                        //recordblackIndex = x;
                                        break;
                                    }
        ...
        /// <returns></returns>
        public PointF GetBoraderPointv2(Bitmap bmp, bool IsXdir, bool IsReverse, Color stopcolor, bool isskip = false)
        {
            Rectangle rectbmp = SimpleRect(bmp.Size);
            BitmapData bmpData = bmp.LockBits(rectbmp, ImageLockMode.ReadWrite, PixelFormat.Format32bppArgb);
            IntPtr Scan0 = bmpData.Scan0;
        ...
        public void TestV3()
        {
            //OpenCvSharp.Cv.CvtScale()
            //OpenCvSharp.Cv.FloodFill()
        }
        ...
        {
            //OpenCvSharp.IplImage source = OpenCvSharp.BitmapConverter.ToIplImage(bmpinput);
            //OpenCvSharp.IplImage image_pattern = OpenCvSharp.BitmapConverter.ToIplImage(bmppattern);
            //OpenCvSharp.IplImage image_result = OpenCvSharp.BitmapConverter.ToIplImage(bmpinput);
            //OpenCvSharp.CvMat cvScalars = new OpenCvSharp.CvMat();
            //image_pattern.MatchTemplate(source, image_result, OpenCvSharp.MatchTemplateMethod.SqDiff);
