...
namespace JzDisplay.OPSpace
{
    ...
    partial class OPDisplay
    {
...
#if (OPT_OLD_VICTOR)
        public OPDisplay(Control pic, Label lbl, float maxratio, float minratio)
        {
            JzMover = new Mover();
        ...
        private void updateInfoLabel()
        {
            ...
            int x = ptMouse_Move.X;
            int y = ptMouse_Move.Y;
            str = $"Position Now : ({x},{y}) ";

            _imgViewer.TransViewportToWorld(ref x, ref y);
            str += $"Mapping Now : ({x},{y}) ";

            var rectWorld = _imgViewer.GetWorldRect();
            if (rectWorld.Contains(x,y))
            {
                Color mcolor = _imgViewer.GetPixel(x,y);
            ...
            if (DISPLAYTYPE != DisplayTypeEnum.CAPTRUE)
                str += "         " + ToSelectListString();

            lblInformation.Text = str.Substring(0, Math.Min(str.Length, 150));
            lblInformation.Invalidate();
#endif
        }
...
#if (OPT_OLD_VICTOR)
        public void GetSearchImage(ref Bitmap bmp)
        {
            ...
            {
                geof?.GenSearchImage(0, 0, bmpOrg, ref bmp, ref bmpx);
            }
            bmpx?.Dispose();
        }
#endif
        ...
        public void SimWheel(int delta = 1)
        {
