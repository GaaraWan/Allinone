...
{
    public class JzMainServiceMachineClass : GeoMachineClass
    {
        const int MSDuriation = 10;

        public JzMainServiceIOClass PLCIO;

        public JzMainServiceMachineClass(Machine_EA machineea, VersionEnum version, OptionEnum option, CameraActionMode actionMode, string opstr, string workpath, bool isnouseplc)
        {
        ...
        public override void GetOPString(string opstr)
        {
            ...
            if (LightCount > 0)
                LightCollection = new JetEazy.ControlSpace.PLCSpace.LightControlClass[LightCount];
        }
        public override bool Initial(bool isnouseio, bool isnousemotor)
        {
            ...
            while (i < LightCount)
            {
                LightCollection[i] = new JetEazy.ControlSpace.PLCSpace.LightControlClass();

                if (!isnouseio)
                    ret &= LightCollection[i].Open(WORKPATH + "\\" + myMachineEA.ToString() + "\\LIGHT" + i.ToString() + ".INI", isnouseio);

                LightCollection[i].Name = "LIGHT" + i.ToString();
                LightCollection[i].ReadAction += ReadAction;

                i++;
            }

            PLCIO = new JzMainServiceIOClass();
        ...
        {
            String Str = new string(readbuffer, 6, 8); //X0000
        ...
        {
            String Str = new string(readbuffer, 6, 8); //Y0000
                                                       // UInt32 GetInt = HEX32(Str);
