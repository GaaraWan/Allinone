...
namespace Common
{
    ...
    public class SideDataClass
    {
        ...
        public void ProcessEX()
        {
            ...
            SearchLine8bitFX(bmptmp, true, SideFoundCollection, 5);
            ...
            {
                int sidefoundcount = sidefound.CountItems();
                if (sidefoundcount > 0)
                {
                    totolcount += sidefoundcount;
                    foundcount++;
                }
            }
            ...
            while (i > -1)
            {
                ...
                {
                    SideFoundCollection.sidefounds[i].ClearItems();
                }
                i--;
            }
    ...
    public class JzFourSideAnalyze
    {
        ...
        public void GetIPDNormalBMP(Bitmap bmpsrc,
            short rgb,
            double thresholdratio,
            double objectfilterratio,
            int edcount,
            ...
            string outrectstr)
        {
            ...
            ContrastStretch contrastStretch = new ContrastStretch();
            Invert invert = new Invert();
            Rectangle rectoutrange = new Rectangle();
            ExtractBiggestBlob extractBiggest = new ExtractBiggestBlob();
            ...
            //畫外框色
            DrawRectOutLine(bmps[bmpindex], 0);
            ...
            i = 0;
            Rectangle bmpcenterrect = new Rectangle(bmpOrg.Width >> 1, bmpOrg.Height >> 1, 10, 10);
            Rectangle bmprect = new Rectangle(0, 0, bmpOrg.Width, bmpOrg.Height);
            ...
            Blob chipblob = chckchipblobs[i];
            //取得晶片的四方型
            Rectangle chiprect = chipblob.Rectangle;    //這個值可做為固定參數
            ...
            foreach (SideDataClass sidedata in SIDEData)
            {
                if (sidedata.SIDEIndex == SIDEEmnum.BOTTOM)
                    i = i;

                sidedata.DrawResultEX(bmpSecond, Color.Lime, 3);
            }
        ...
        /*
        void ProcessIPDNormal(Bitmap bmp,
            double thresholdratio,
            double objectfilterratio,
            int edcount,
            double shortenratio,
            Rectangle outrect)
        {
            ...
            //排名第2的是晶片
            Blob chipblob = blobs[int.Parse(sortlist[1].Split(',')[1])];
            //取得晶片的四方型
            Rectangle chiprect = chipblob.Rectangle;
    ...
    public class JzSideAnalyzeEXClass
    {
        ...
        {
            jzapara = new JzAnalyzeParaEXClass();
            bmps = new Bitmap[bmpcount];
        ...
        public void Train(JzAnalyzeParaEXClass para, Bitmap bmpsrc, List<Bitmap> bmplist)
        {
            ...
            jzapara.GetPara(para);
            jzapara.GetBangBangSide(rectwhole);

            //取得chip的尺吋
            jzapara.SetChipRatioRect = GetTrainedChipRect(bmpsrc, bmplist);

            Bitmap bmp = bmplist[0];

            DrawRectOutside(bmp, Color.LimeGreen, 3, jzapara.rectChipRatio);

            //取得各個邊要計算的方框尺吋
            while (i < (int)SIDEEmnum.COUNT)
            {
                Rectangle rect = jzapara.GetSideDefinedRect((SIDEEmnum)i, rectwhole);
                sidedataexs[i] = new SideDataEXClass((SIDEEmnum)i, rect);

                DrawRectOutside(bmp, Color.LimeGreen, 3, rect);

                i++;
            }

            //SaveAllbmps(bmplist, "TRAINED");
        ...
        Rectangle GetNormalChipRect(Bitmap bmpsrc, List<Bitmap> bmplist)
        {
            ...
            {
                if (bb.Rectangle.IntersectsWith(rectcenter))
                {
                    Rectangle orgrect = bb.Rectangle;

                    orgrect.Inflate(20, 20);
                    orgrect.Intersect(rectwhole);

                    if (orgrect != rectwhole)
                    {
                        break;
                    }
                }
            ...
            DrawRectOutside(bmp, Color.Red, 3, rectret);
            bmplist.Add(bmp); //bmplist[0]
            //SaveAllbmps(bmplist, "TRAINNORMAL");
        ...
        public string RunIPDCheck(Bitmap bmpsrc, List<Bitmap> bmplist)
        {
            ...
            if (jzapara.RectBangBang != new Rectangle())
            {
                ...
                bmps[bmpindex + 1] = threshold.Apply(bmps[bmpindexTobeThreshold]);
                bmpindex++;
        ...
        public string RunNormalCheck(Bitmap bmpsrc, List<Bitmap> bmplist)
        {
            ...
            while (i < (int)SIDEEmnum.COUNT)
            {
                sidedataexs[i].ProcessEX(bmps[bmpindex]);
                i++;
            }
            //畫出所有的點點
            i = 0;
            while (i < (int)SIDEEmnum.COUNT)
            {
                sidedataexs[i].DrawResultEX(bmps[0], Color.LimeGreen, 3);
                i++;
            }
        ...
        void DrawBangBang(Bitmap bmpsrc, Rectangle rect)
        {
        ...
        public void SaveAllbmps(List<Bitmap> bmplist, string headstr)
        {
            string filenames = "";

            Bitmap[] bmps = new Bitmap[bmplist.Count];
    ...
    /// 找線條的參數
    /// </summary>
    public class JzAnalyzeParaEXClass
    {
        //直接指定的參數
        public MethodEnum mymethod;
        public short rgb;

        public int bangbangoffsetval = 0;
        public int edcount;
        public int shrinkratio;
        public int objfilterratio;
        public int threshodratio;

        public Rectangle rectbangbang;
        public Rectangle rectneveroutrange;

        public SIDEEmnum sideBangBang = SIDEEmnum.TOP;

        //依照指定的參數所得出的
        public Rectangle rectChipRatio;
        ...
        public double ObjetFilteRatio
        {
            get
            {
                return (double)objfilterratio / 100d;
            }
        }
        public int EDCount
        {
        ...
        public Rectangle SetChipRatioRect
        {
            set
            {
                Rectangle rect = value;

                int swidth = (rect.Width - (int)(rect.Width * ShrinkRatio)) / 2;
                int sheight = (rect.Height - (int)(rect.Height * ShrinkRatio)) / 2;

                rect.Inflate(-swidth, -sheight);
        ...
        public double ThresholdRatio
        {
            get
            {
                return (double)threshodratio / 100d;
            }
        }
        public double ShrinkRatio
        {
            get
            {
                return (double)shrinkratio / 100d;
            }
        }
        public int BangBangOffsetVal
        {
            get
            {
                return bangbangoffsetval;
            }
        }
        public JzAnalyzeParaEXClass()
        {
        ...
        public void GetPara(JzAnalyzeParaEXClass para)
        {
            mymethod = para.mymethod;
            rgb = para.rgb;
            bangbangoffsetval = para.bangbangoffsetval;

            edcount = para.edcount;
            shrinkratio = para.shrinkratio;
            objfilterratio = para.objfilterratio;
            threshodratio = para.threshodratio;
            rectbangbang = para.rectbangbang;
            rectneveroutrange = para.rectneveroutrange;
        ...
        /// 取得上下左右區域的方形位置及尺吋
        /// </summary>
        /// <param name="side"></param>是位於何位的尺吋
        ...
        /// 確定棒棒的預設位置
        /// </summary>
        /// <param name="rectwhole"></param>
        public void GetBangBangSide(Rectangle rectwhole)
        {
            SIDEEmnum bangbangside = SIDEEmnum.TOP;

            if (rectbangbang.Width > rectbangbang.Height) //棒棒是否位於上下左右位置
            {
                if (rectbangbang.Y < (rectwhole.Y + (rectwhole.Height >> 1)))
                    bangbangside = SIDEEmnum.TOP;
                else
                    bangbangside = SIDEEmnum.BOTTOM;
            }
            ...
            {
                if (rectbangbang.X < (rectwhole.X + (rectwhole.Width >> 1)))
                    bangbangside = SIDEEmnum.LEFT;
                else
                    bangbangside = SIDEEmnum.RIGHT;
            }
            sideBangBang = bangbangside;
        }

        public bool CheckSideBangBang(Rectangle rectcheckbangbagn)
        {
            bool ret = true;
            ...
            {
                case SIDEEmnum.TOP:
                    if (rectbangbang.Top - rectcheckbangbagn.Top > bangbangoffsetval)
                    {
                        ret = false;
                    }
                    break;
                case SIDEEmnum.BOTTOM:
                    if (rectcheckbangbagn.Bottom - rectbangbang.Bottom > bangbangoffsetval)
                    {
                        ret = false;
                    }
                    break;
                case SIDEEmnum.LEFT:
                    if (rectbangbang.Left - rectcheckbangbagn.Left > bangbangoffsetval)
                    {
                        ret = false;
                    }
                    break;
                case SIDEEmnum.RIGHT:
                    if (rectcheckbangbagn.Right - rectbangbang.Right > bangbangoffsetval)
                    {
                        ret = false;
                    }
                    break;
        ...
        public void FromString(string str)
        {
            ...
            mymethod = (MethodEnum)int.Parse(strs[0]);
            rgb = short.Parse(strs[1]);
            bangbangoffsetval = int.Parse(strs[2]);
            edcount = int.Parse(strs[3]);
            shrinkratio = int.Parse(strs[4]);
            objfilterratio = int.Parse(strs[5]);
            threshodratio = int.Parse(strs[6]);

            rectbangbang = StringtoRect(strs[7]);
            rectneveroutrange = StringtoRect(strs[8]);
            sideBangBang = (SIDEEmnum)int.Parse(strs[9]);
            rectChipRatio = StringtoRect(strs[10]);
        ...
        public override string ToString()
        {
            ...
            retstr += ((int)mymethod).ToString() + seperator;
            retstr += rgb.ToString() + seperator;
            retstr += bangbangoffsetval.ToString() + seperator;
            retstr += edcount.ToString() + seperator;
            retstr += shrinkratio.ToString() + seperator;
            retstr += objfilterratio.ToString() + seperator;
            retstr += threshodratio.ToString() + seperator;

            retstr += RecttoString(rectbangbang) + seperator;
            retstr += RecttoString(rectneveroutrange) + seperator;

            retstr += ((int)sideBangBang).ToString() + seperator;
            retstr += RecttoString(rectChipRatio) + seperator;
    ...
    /// 邊緣資料
    /// </summary>
    public class SideDataEXClass
    {
        SIDEEmnum myside = SIDEEmnum.TOP;
        const int foundcount = 50;
        public SideFoundEXClass[] sidefounds;

        Rectangle rectside = new Rectangle();

        public SideDataEXClass(SIDEEmnum side, Rectangle rect)
        {
            int i = 0;
            ...
            sidefounds = new SideFoundEXClass[foundcount];

            while (i < foundcount)
            {
                sidefounds[i] = new SideFoundEXClass();
                i++;
            }
        }

        public void ProcessEX(Bitmap bmpsrc)
        {
            Suicide();

            Bitmap bmp = CopyImage(bmpsrc, rectside);
            ...
            foreach (SideFoundEXClass sidefound in sidefounds)
            {
                int sidefoundcount = sidefound.CountItems();
                if (sidefoundcount > 0)
                {
            ...
            while (i > -1)
            {
                if (sidefounds[i].CountItems() < checkmean)
                {
                    sidefounds[i].ClearItems();
                }
                i--;
        ...
        void SearchLine8bitFX(Bitmap bmpsrc, bool ischeckwhite, SIDEEmnum side, SideFoundEXClass[] sidefounds, int dupgap, int gap = 1)
        {
            byte CheckColor = 0;
            ...
            Rectangle rectbmp = new Rectangle(0, 0, bmpsrc.Width, bmpsrc.Height);
            BitmapData bmpData = bmpsrc.LockBits(rectbmp, ImageLockMode.ReadWrite, PixelFormat.Format8bppIndexed);
                                    ...
                                    if (*pucPtr == CheckColor)
                                    {
                                        if (CheckColor == 255)
                                        {
                                            *pucPtr = 100;
                                            ...
                                            SideFoundItemEXClass sidefounditem = new SideFoundItemEXClass(new Point(x, y), Math.Abs(y - lastval));
                                            lastval = y;

                                            sidefounds[sidefoundindex].Add(sidefounditem);
                                    ...
                                    if (*pucPtr == CheckColor)
                                    {
                                        if (CheckColor == 255)
                                        {
                                            *pucPtr = 100;
                                            ...
                                            SideFoundItemEXClass sidefounditem = new SideFoundItemEXClass(new Point(x, y), Math.Abs(x - lastval));
                                            lastval = x;
        ...
        public void DrawResultEX(Bitmap bmp, Color color, int width, bool isnew = false)
        {
            ...
            {
                Point[] pts = new Point[sidedfound.CountItems()];
                sidedfound.FillPtsEX(ref pts, rectside.Location, isnew);
        ...
        public void ProcessDistance(ref string resultstr)
        {
            ...
            resultstr += myside.ToString() + Environment.NewLine;
        ...
        public Point[] GetPoints(int index = 0)
        {
            ...
            if (index > 0)
            {
                ...
                {
                    SideFoundEXClass sidedfoundx = sidefounds[1];
                    pts = new Point[sidedfoundx.CountItems()];
                    sidedfoundx.FillPtsEX(ref pts, rectside.Location);
                }
            }
            else
            {
                ...
                {
                    SideFoundEXClass sidedfound = sidefounds[index];
                    pts = new Point[sidedfound.CountItems()];
                    sidedfound.FillPtsEX(ref pts, rectside.Location);
    ...
    /// 邊綠找到的線條點集合
    /// </summary>
    public class SideFoundEXClass
    {
        List<SideFoundItemEXClass> founditemList = new List<SideFoundItemEXClass>();

        public int MaxVal = -100000;
        public int MinVal = 100000;

        public SideFoundEXClass()
        {

        }
        public void Add(SideFoundItemEXClass item)
        {
        ...
        public void FillPtsEX(ref Point[] pts, Point offsetpt, bool isnew = false)
        {
            ...
            pts = new Point[founditemList.Count];

            int i = 0;
            while (i < pts.Length)
            {
                pts[i] = founditemList[i].ptStart;
                pts[i].Offset(offsetpt);
        ...
        public void FillDistance(List<int> dist, ref int max, ref int min, ref double mean)
        {
            ...
            foreach (SideFoundItemEXClass item in founditemList)
            {
                dist.Add(item.length);
        ...
        public void NGPtsFilter(double rangeratio)
        {
            ...
            foreach (SideFoundItemEXClass sidefound in founditemList)
            {
                histodata[(sidefound.length - MinVal)]++;
            }
            ...
            i = founditemList.Count - 1;

            while (i > -1)
            {
                if (founditemList[i].length > upperval || founditemList[i].length < lowerval)
                {
                    founditemList.RemoveAt(i);
                }
                i--;
            }
        }
    ...
    /// 線條點
    /// </summary>
    public class SideFoundItemEXClass
    {
        public Point ptStart = new Point(0, 0);
        public int length = 0;

        public SideFoundItemEXClass(Point pt, int len)
        {
            ptStart = pt;
            length = len;
        }
        public Point GetOffset(Point pt)
        {
            Point retpt = pt;
