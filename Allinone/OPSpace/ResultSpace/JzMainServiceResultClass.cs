...
{
    public class JzMainServiceResultClass : GeoResultClass
    {
     ...
     //   public string RELATECOLORSTR = "";
        ...
        public JzMainServiceResultClass(Result_EA resultea,VersionEnum version,OptionEnum option,MachineCollectionClass machinecollection)
        {
            myResultEA = resultea;
            VERSION = version;
            OPTION = option;
            ...
            MACHINE = (JzMainServiceMachineClass)machinecollection.MACHINE;
        ...
        public void FillProcessImageMainServiceCal()
        {
            EnvClass env = AlbumWork.ENVList[EnvIndex];
            if (IsNoUseCCD)
            {
                ...
                //{
                //    page.SetbmpRUN(PageOPTypeEnum.P00, CCDCollection.GetBMP(i, false));
                //    page.A08_RunServiceProcess(PageOPTypeEnum.P00);
                //    break;
                //    i++;
                //}

                Parallel.ForEach(env.PageList, page =>
                {
                    page.SetbmpRUN(PageOPTypeEnum.P00, CCDCollection.GetBMP(page.No, false));
                    page.A08_RunServiceProcess(PageOPTypeEnum.P00);
                });
                ...
                switch (Process.ID)
                {
                    ...
                    case 10100:                        //變換CCD亮度設定及光源設定，並且合起鍵盤的壓框
                        if (Process.IsTimeup)
                        {
                            ...
                            LogProcessIDTimer(Process.ID, "线程创建" + CamActClass.Instance.StepCurrent.ToString());
                            //计算线程
                            ...
                            LogProcessIDTimer(Process.ID, "线程测试" + CamActClass.Instance.StepCurrent.ToString());
                ...
                switch (Process.ID)
                {
                    ...
                    case 20:
                        if (Process.IsTimeup)       //抓圖
                        {
                            ...
                            //FillProcessImage();
                            FillProcessImageMainServiceCal();
                    ...
                    case 30:
                        if (Process.IsTimeup)
                        {
                            ...
                            foreach (PageClass page in env.PageList)
                            {
                                //if (!string.IsNullOrEmpty(page.RunStatusStr))
                                //{

                                //}

                                if (page.AnalyzeRootArray[(int)PageOPTypeEnum.P00].RunStatusCollection.COUNT > 0)
                                {
                                    foreach (WorkStatusClass workStatus in page.AnalyzeRootArray[(int)PageOPTypeEnum.P00].RunStatusCollection.WorkStatusList)
                                    {
