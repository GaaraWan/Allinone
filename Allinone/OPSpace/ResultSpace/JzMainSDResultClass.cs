...
{
    public class JzMainSDResultClass : GeoResultClass
    {
     ...
     //   public string RELATECOLORSTR = "";
        ...
        public JzMainSDResultClass(Result_EA resultea,VersionEnum version,OptionEnum option,MachineCollectionClass machinecollection)
        {
            myResultEA = resultea;
            VERSION = version;
            OPTION = option;
            ...
            MACHINE = (JzMainSDMachineClass)machinecollection.MACHINE;
                ...
                switch (Process.ID)
                {
                    ...
                    case 1030:                        //拍片完後關燈
                        if (Process.IsTimeup)
                        {
                            ...
                            {
                                case TestMethodEnum.IO:
                                    //MACHINE.IsGetImageComplete = true;
                                    //MACHINE.PLCIO.PCGetImageComplete = true;
                                    break;
                                case TestMethodEnum.BUTTON:
                                    CCDCollection.GetImage();
                                    break;
                            ...
                            //              CCDCollection.GetImageEX("0,1,2,3");
                            //MACHINE.PLCIO.PCGetImageComplete = true;

                            //OnTrigger(ResultStatusEnum.GETIMAGECOMPLETE);
                            FillProcessImage();
                        ...
                        if (Process.IsTimeup)
                        {
                            ...
                            switch (TestMethod)
                            {
                                case TestMethodEnum.IO:
                                    ...
                                    //m_testpass = !m_testpass;
                                    //IsPass = m_testpass;

                                    //IsPass = true;

                                    JzMainSDPositionParas.INSPECT_RESULT = IsPass;

                                    //if (!MACHINE.PLCIO.ADR_ISEMC)
                                    {
                                        if (IsPass)
                                        {
                                            MACHINE.PLCIO.TAKE_PASS = true;
                                            MACHINE.PLCIO.TAKE_NG = false;
                                        }
                                        else
                                        {
                                            MACHINE.PLCIO.TAKE_PASS = false;
                                            MACHINE.PLCIO.TAKE_NG = true;
                                        }
                                    }

                                    //MACHINE.IsGetImageComplete = true;//这个可能要移到后面 这样资料才会全。
        ...
        void RXXLastProcess()
        {
            ...
            switch (TestMethod)
            {
                case TestMethodEnum.IO:
                    break;
                default:
                    JzMainSDPositionParas.ReportMANUALSave(IsPass);
                    break;
            }
